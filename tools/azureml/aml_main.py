...
def main():
    ...
    cmd_run(['ln', '-s',
             data_dir,
             op.join(working_dir, 'datasets')])

    cmd_run(['ln', '-s',
             args.output_dir,
             op.join(working_dir, 'output')])

    cmd_run(['ln', '-s',
             model_dir,
             op.join(working_dir, 'models')])
    ...
    with open(op.join(working_dir, 'output', 'build_apex.log'), 'w') as output_f:
        cmd = 'python setup.py build develop'
        cmd_run(cmd, working_directory=op.join(working_dir, 'apex'), output=output_f)
