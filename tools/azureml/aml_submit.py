...
def parse_args():
    ...
    parser.add_argument('--job_name', required=False, type=str, default='test',
                        help='job name for job submission')
    ...
    parser.add_argument('--exp_name', type=str, default='output', 
                        help='output relative path w.r.t. datastore')
...
if __name__ == '__main__':
    ...
    For example: 
    python tools/azureml/aml_submit.py --config-file train_oid.yaml 
    ...
    job_name = "{}_{}".format(datetime.datetime.now().strftime("%Y%m%d_%H%M"), args.job_name)
    output_dir = op.join(args.output_dir, job_name)
    ...
    output_dir_local = op.join(args.output_dir, "{}_{}".format(run_number, job_name))
    mkdir(output_dir_local)
    with open(os.path.join(output_dir_local, 'submit_config.json'), 'w') as f:
        json.dump(vars(args), f)

    with open(os.path.join(output_dir_local, 'experiment_config.json'), 'w') as f:
